            ...
            <div class="marketingContent">
              <div class="marketingLft">
                ...
                <div class="articleCopyright">
                  ...
                  <span class="visit">浏览次数：{{infoVisit}}</span>
                  <span class="updateTime">更新时间：{{info.updateTime}}</span></div>
                <div class="articleDetail">【内容提要】{{info.note}}</div>
                <div class="articleCoverPic"><img v-lazy="picUrl" v-if="picUrl" alt=""
                                                  style="margin: 10px auto; padding: 5px; list-style: none; vertical-align: middle; border: 1px solid rgb(221, 221, 221); border-radius: 3px; box-shadow: rgba(0, 0, 0, 0.15) 0px 0px 8px; background: rgb(251, 251, 251); color: rgb(51, 51, 51); font-family: &quot;Microsoft Yahei&quot;, verdana; font-size: 14px; white-space: normal;height: 250px;max-width:80%">
                </div>
                <div class="articleContent" v-html='info.content'>
                </div>
                <div class="articleAnnex" v-if="fileUrl">附件:<a :href="fileUrl" :download="fileUrl">{{fileTitle}}</a></div>
                <ul class="articleUl">
                  <li>
                    <div class="relatedTitle pre" v-if="post_pre">
                      上一篇：
                      <a href=''>
                        <router-link :to="{path:'/detail', query:{type: preInfo.type, id: preInfo.id}}">
                          {{preInfo.title}}
                        </router-link>
                      </a>
                    </div>
                  ...
                  <li>
                    <div class="relatedTitle next" v-if="post_next">
                      下一篇：
                      <a href=''>
                        <router-link :to="{path:'/detail', query:{type: nextInfo.type, id: nextInfo.id}}">
                          {{nextInfo.title}}
                        </router-link>
                      </a>
                    </div>
              ...
              <div class="marketingRgt">
                ...
                <ul class="marketingArticle" v-for="(hotInfo, i) in hotList">
                  <li><a href=''>
                    <router-link :to="{path:'/detail', query:{type: hotInfo.type, id: hotInfo.id}}">{{hotInfo.title}}
                    </router-link>
                  </a></li>
                </ul>
                <!--<div class="hot_news_title">-->
  ...
  export default {
    ...
    methods: {
      getData() {
        ...
        selectInfoByIdAndType(JSON.stringify(postData)).then(res => {
          _this.info = res.data.resultList[0]
          _this.info.updateTime = formatDate(_this.info.updateTime, "yyyy-MM-dd")
          _this.preInfo = res.data.preText[0]
          _this.nextInfo = res.data.nextText[0]
          _this.picList = _this.info.picList
          _this.fileList = _this.info.fileList
          if (_this.preInfo !== undefined) {
            _this.post_pre = res.status
          } else {
            _this.err_pre = res.status
          }
          if (_this.nextInfo !== undefined) {
            _this.post_next = res.status
          } else {
            _this.err_next = res.status
          }
          if (_this.picList.length !== 0) {
            _this.picUrl = this.serverUrl + '\\images\\' + _this.picList[0].picUrl
          }
          if (_this.fileList.length !== 0) {
            _this.fileUrl = this.serverUrl + '\\files\\' + _this.fileList[0].fileUrl
            _this.fileTitle = _this.fileList[0].fileTitle
          }
          // if (_this.info.coverPic !== 119) {
          //   selectPictureById(_this.info.coverPic).then(res => {
    ...
    watch: {
      "$route": function (to, from) {
        ...
        // console.log(to, from);
        let url = to.fullPath;
        this.info_type = url.split("&")[0].split("=")[1];
        this.info_id = url.split("&")[1].split("=")[1];
        this.picUrl = ''
        this.fileUrl = ''
        this.getData()
        this.getHotList()
...
<style scoped lang="scss" type="text/scss">
  ...
  .marketingList ul li {
    /*background:url(../../assets/images/dot3.jpg) 3px center no-repeat;*/
    padding-left: 13px;
